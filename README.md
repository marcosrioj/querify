...
- Configure `Ai:UserId` in both AI API appsettings files.
- Use the id generated by the seed action `Seed essential data (IA Agent user only)`.
...
- Essential seed: ensures the IA Agent user exists for AI API usage.
...
- Essential seed prints the IA Agent `UserId`; copy this value to `Ai:UserId` in:
  `dotnet/BaseFaq.AI.Generation.Api/appsettings.json` and `dotnet/BaseFaq.AI.Matching.Api/appsettings.json`.
