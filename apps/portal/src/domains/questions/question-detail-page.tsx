...
export function QuestionDetailPage() {
  ...
  const [selectedSourceRole, setSelectedSourceRole] = useState(String(SourceRole.SupportingContext));
  const [selectedAnswerId, setSelectedAnswerId] = useState('');
  const [selectedDuplicateId, setSelectedDuplicateId] = useState('');

  const availableTags = useMemo(() => {
    const existing = new Set((questionQuery.data?.tags ?? []).map((tag) => tag.id));
    return (tagOptionsQuery.data?.items ?? []).filter((tag) => !existing.has(tag.id));
  }, [questionQuery.data?.tags, tagOptionsQuery.data?.items]);

  const availableSources = useMemo(() => {
    const existing = new Set((questionQuery.data?.sources ?? []).map((link) => link.sourceId));
  ...
  const duplicateOptions = useMemo(
    () =>
      (relatedQuestionOptionsQuery.data?.items ?? []).filter((question) => question.id !== id),
          ...
          <Card>
            <CardContent className="grid grid-cols-2 gap-2 p-3">
              ...
              <ConfirmAction
                ...
                onConfirm={() =>
                  deleteQuestion.mutateAsync(id).then(() => navigate('/app/questions'))
                }
                trigger={
                  <Button
                    variant="destructive"
          ...
          {showLoadingState ? (
            ...
            <Card>
              ...
              <CardContent>
                <KeyValueList
                  items={[
                    { label: 'Space', value: spaceQuery.data?.name ?? questionQuery.data.spaceKey },
                    {
        ...
        <>
          <SectionGrid
            items={[
              {
                title: 'Status',
                value: <QuestionStatusBadge status={questionQuery.data.status} />,
                icon: MessageSquareText,
              },
              {
                title: 'Visibility',
                value: <VisibilityBadge visibility={questionQuery.data.visibility} />,
                icon: MessageSquareText,
              },
              {
                title: 'Signals',
                ...
                description: translateText('Public feedback is aggregated into activity and score'),
                icon: Activity,
              },
              {
                title: 'Answers',
                value: questionQuery.data.answers.length,
                description: translateText('Accepted, draft, and review candidates'),
                icon: CheckCircle2,
              },
            ]}
          />
          ...
          <Card>
            <CardHeader>
              <CardHeading>
                <CardTitle>{translateText('Accepted answer and duplicate routing')}</CardTitle>
              </CardHeading>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid gap-4 md:grid-cols-2">
              ...
              <CardHeading>
                <CardTitle className="flex items-center gap-2">
                  ...
                  <Badge variant="outline">
                    {translateText('{count} tags', { count: questionQuery.data.tags.length })}
                  </Badge>
                </CardTitle>
              </CardHeading>
            </CardHeader>
                    ...
                    >
                      <div className="min-w-0">
                        <p className="font-medium text-mono">
                          {sourceLink.source?.label || sourceLink.source?.locator || sourceLink.sourceId}
                        </p>
                      ...
                      <div className="flex flex-wrap gap-2">
                        ...
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => void removeSource.mutateAsync(sourceLink.id)}
                        >
                          <Trash2 className="size-4" />
                  ...
                  >
                    <div className="min-w-0">
                      ...
                      <div className="mt-2 flex flex-wrap items-center gap-2">
                        ...
                        {answer.isOfficial ? (
                          <Badge variant="primary">{translateText('Official')}</Badge>
                        ) : null}
                        <Badge variant="outline">
          ...
          <Card>
                ...
                <CardTitle className="flex items-center gap-2">
                  ...
                  <Badge variant="outline">
                    {translateText('{count} events', {
                      count:
                        (activityListQuery.data?.items ?? questionQuery.data.activity).length,
                    })}
                  </Badge>
                </CardTitle>
              </CardHeading>
            ...
            <CardContent className="space-y-3">
              {((activityListQuery.data?.items ??
                questionQuery.data.activity) as ActivityDto[]).length ? (
                ((activityListQuery.data?.items ??
                  questionQuery.data.activity) as ActivityDto[]).map((event) => (
                  <div
                    key={event.id}
                    className="flex flex-col gap-3 rounded-2xl border border-border bg-muted/10 p-4 sm:flex-row sm:items-start sm:justify-between"
                  >
