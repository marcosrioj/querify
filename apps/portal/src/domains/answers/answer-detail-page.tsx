...
export function AnswerDetailPage() {
  ...
  const [selectedSourceRole, setSelectedSourceRole] = useState(String(SourceRole.Evidence));

  const availableSources = useMemo(() => {
    const existing = new Set((answerQuery.data?.sources ?? []).map((link) => link.sourceId));
          ...
          <Card>
            <CardContent className="grid grid-cols-2 gap-2 p-3">
              ...
              <ConfirmAction
                ...
                onConfirm={() =>
                  deleteAnswer.mutateAsync(id).then(() => navigate('/app/answers'))
                }
                trigger={
                  <Button
                    variant="destructive"
        ...
        <>
          <SectionGrid
            items={[
              ...
              {
                title: 'Visibility',
                value: <VisibilityBadge visibility={answerQuery.data.visibility} />,
                icon: CheckCircle2,
              },
              {
                title: 'Type',
