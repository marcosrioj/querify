...
Sources, accepted answers, and answer follow-up questions are optional relationship features. They can add provenance, curation, canonical selection, and recursive navigation, but they must not block setup progress or become mandatory next actions.
...
| [`answer-detail-page.tsx`](../../../apps/portal/src/domains/answers/answer-detail-page.tsx) | If the Answer is draft, show `Activate answer`. If archived, show `Reactivate answer`. Else show `Review activity`. Missing source links, follow-up questions, or accepted state must not drive the recommended action. |
...
| [`answer-form-page.tsx`](../../../apps/portal/src/domains/answers/answer-form-page.tsx) | `Question`: value exists. `Headline`: at least 3 trimmed characters. `Answer kind`: value exists. `Visibility`: value exists. Follow-up questions are optional and do not affect setup completion. |
...
| [`answer-detail-page.tsx`](../../../apps/portal/src/domains/answers/answer-detail-page.tsx) | No sources, follow-up questions, or answer activity -> explain optional references, recursive navigation, and lifecycle history expectations. |
...
5. Keep Source links, Answer links, follow-up questions, and accepted-answer selection optional unless the backend contract changes.
