...
public sealed class TestDatabase : IDisposable
{
    ...
    public static TestDatabase Create(string databasePrefix)
    {
        var config = TestDatabaseConfig.Load();
        var normalizedPrefix = string.IsNullOrWhiteSpace(databasePrefix)
            ? "bf_ai_test"
            : databasePrefix.Trim().ToLowerInvariant();
