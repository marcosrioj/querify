...
flowchart TD
    ...
    Import --> Create
    Other --> Create
    Create[Create Question inside Space] --> Classify[Set QuestionKind, Visibility, ProductScope, JourneyScope, AudienceScope, ContextKey]
    Classify --> Capture[Register origin as Source + QuestionSourceLink]
    ...
    OpenThread --> Activity[Append Activity events]
```
...
| --- | --- | --- |
| [Space](../Domain/Space.cs) | Supplies the operating rules for incoming threads. | `ModerationPolicy`, `AcceptsQuestions`, `RequiresQuestionReview`, `Visibility` |
...
| [Source](../Domain/Source.cs) | Raw source record for imports, tickets, chats, pages, and other origins. | `Kind`, `Locator`, `SystemName`, `ExternalId`, `MetadataJson`, `CapturedAtUtc` |
...
| [Activity](../Domain/Activity.cs) | Stores the audit events for creation, submission, approval, rejection, or escalation. | `Kind`, `ActorKind`, `ActorLabel`, `Notes`, `MetadataJson`, `OccurredAtUtc` |
...
- `Space` owns the rules, but `Question` carries the operational state.
- The intake channel is stored directly on `Question.OriginChannel`, while raw provenance lives in `Source` plus `QuestionSourceLink`.
- Rejection is modeled through `Activity`, not through a dedicated `QuestionStatus.Rejected` enum value.
- `Question.Tags` is classification after creation; it is not the primary container. The primary container is still `Space`.
