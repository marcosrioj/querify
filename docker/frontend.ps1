...
docker compose -p bf_services -f $ComposeFile stop $Service 2>$null | Out-Null
docker compose -p bf_services -f $ComposeFile rm -f $Service 2>$null | Out-Null

Write-Banner "BaseFaq Frontend Docker Images..."

$images = docker images --format '{{.Repository}} {{.ID}}' |
  Where-Object { $_ -match '^basefaq\.portal\.app\s+' } |
...
docker compose -p bf_services -f $ComposeFile up -d --build $Service
